...
class Sequential_ext(nn.Module):
    ...
    def forward(self, input, temperature=1, gate_mode='stochastic', prob=1, openings=None):
        gate_activations = []
        for i, module in enumerate(self._modules.values()):
            input, gate_activation = module(input, temperature, gate_mode, prob)
...
#        print 'fc before gumble', w.shape
        ...
        if gate_mode == "argmax":
          ...
          output_multiplier = torch.ones(x.size())
        else:
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x, temperature=1, gate_mode='stochastic', prob=1):
        assert(gate_mode in ['stochastic', 'always_on', 'argmax', 'stochastic-variable'])
...
#            print 'w_soft.shape', w_soft.shape
            output_multiplier = torch.autograd.Variable(torch.ones(out.size()).cuda(), volatile=True)# * w_soft[:,1].unsqueeze(1)
...
class ResNet_ImageNet(nn.Module):
    ...
    def forward(self, out, temperature=1, gate_mode = 'stochastic', prob=1, threshold=.5):
        ...
        out, a = self.layer1(out, temperature, gate_mode, prob)
        gate_activations.extend(a)
        out, a = self.layer2(out, temperature, gate_mode, prob)
        gate_activations.extend(a)
        out, a = self.layer3(out, temperature, gate_mode, prob)
        gate_activations.extend(a)
        out, a = self.layer4(out, temperature, gate_mode, prob)
        gate_activations.extend(a)
